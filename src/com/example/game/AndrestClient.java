...
public class AndrestClient {
	...
	/**
	 * Main controller for the client, has the ability to create any of the
	 * other methods. Call with your connection type and data, and everything is
	 * handled for you.
	 * 
	 * @param url
	 *            the url you wish to call
	 * @param method
	 *            the method you wish to call wish
	 * @param data
	 *            the data you want to pass to the URL, can be null
	 * @return JSON the JSONObject returned from the request
	 */
	public JSONObject request(String url, String method,
			Map<String, Object> data) throws RESTException {
	...
	/**
	 * Calls a GET request on a given url. Doesn't take a data object (yet), so
	 * pass all get parameters alongside the url.
	 * 
	 * @param url
	 *            the url you wish to connect to
	...
	public JSONObject get(String url) throws RESTException {
		...
		try {
			...
			if (statusCode != 200) {
				throw new Exception(
						"Error executing GET request! Received error code: "
	...
	/**
	 * Calls a POST request on a given url. Takes a data object in the form of a
	 * HashMap to POST.
	 * 
	 * @param url
	 *            the url you wish to connect to
	 * @param data
	 *            the data object to post to the url
	 * @return JSON the JSON response from the call
	 */
	public JSONObject post(String url, Map<String, Object> data)
		...
		try {
			...
			for (int i = 0; i < response.getAllHeaders().length; i++) {
				Log.i("Savio", " response name:  " + headers[i].getName()
						+ " response value: " + headers[i].getValue());
			}

			int statusCode = response.getStatusLine().getStatusCode();
			if (statusCode != 200) {
				throw new Exception(
						"Error executing POST request! Received error code: "
								+ response.getStatusLine().getStatusCode());
			}
	...
	/**
	 * Calls a PUT request on a given url. Takes a data object in the form of a
	 * HashMap to PUT.
	 * 
	 * @param url
	 *            the url you wish to connect to
	 ...
	 * @return JSON the JSON response from the call
	 */
	public JSONObject put(String url, Map<String, Object> data)
			throws RESTException {
		...
		try {
			...
			if (statusCode != 200) {
				throw new Exception(
						"Error executing PUT request! Received error code: "
								+ response.getStatusLine().getStatusCode());
			}
	...
	public JSONObject delete(String url) throws RESTException {
		...
		try {
			...
			if (statusCode != 200) {
				throw new Exception(
						"Error executing DELETE request! Received error code: "
								+ response.getStatusLine().getStatusCode());
			}
	...
	/**
	 * Adds a set of cookies to the HTTPClient. Pass in a HashMap of <String,
	 * Object> to add the cookie values to the client. These cookies persist
	 * until removed.
	 * 
	 * @param cookies
	 *            a HashMap of cookies
	 * @param url
	 *            the domain URL of the cookie
	 */
	public void setCookies(HashMap<String, String> cookies, String url) {
		...
		// Create cookie store
		BasicCookieStore store = (BasicCookieStore) (client.getCookieStore() == null ? new BasicCookieStore()
	...
	/**
	 * Helper method for creating an error object for better reporting. Not
	 * needed, but has proven useful before and can be used to pass objects to
	 * the error class RESTException.
	 * 
	 * @param keys
	 *            a list of keys
	 * @param values
	 ...
	 * Used alongside POST and PUT requests to neaten the parameters, rather
	 * than attempting to stringify the data.
	 * 
	 * @param data
	 *            the data object we're going to pass
	 ...
	 * 
	 * @param is
	 *            the InputStream we're reading, usually from
